...
class LightningModel(L.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        loss, predicted_value = self._shared_step(batch)

        self.log("train_loss", loss)
        self.train_mse.update(predicted_value, batch[1])
        self.log(
    ...
    def validation_step(self, batch, batch_idx):
        ...
        self.val_mse.update(predicted_value, batch[1])
        self.log("val_mse", self.val_mse, prog_bar=True)

    def test_step(self, batch, batch_idx):
        loss, predicted_value = self._shared_step(batch)
        self.test_mse.update(predicted_value, batch[1])
